...
(function($){
	...
	var template = '<div class="cP1 cClear" id="ID_CONTAINER">'+
		'<div id="ID_SCROLLER" class="scrollablePane cH98 cClear ui-widget ui-state-default ui-corner-all whitebg" align="center">'+
		...
		'&nbsp;<input id="ID_PAGENO" type="text" class="" onblur="$(\'#ID\').dfPagerUI(\'enableAll\')" onfocus="this.value=$(\'#ID\').dfPagerUI(\'getPageNo\')" onchange="$(\'#ID\').dfPagerUI(\'gotoPage\',parseInt(this.value))" size="4" value=""/>'+
	...
	var Methods = {
		...
		getPageCount: function() {
			var id = this.attr('id');
			return this.dfPagerUI.pagers[id].pageCount;
		},
		getPageNo: function() {
		...
		setResultCount: function(total) {
			this.dfPager('setResultCount',total);
		},
		getResultCount: function() {
			return this.dfPager('getResultCount');
		},
		...
		init : function(opts) {
			...
			if(id) {
				...
				function SearchContainerSize() {
					$("#"+id+"_CONTAINER").css("height","50px");
					var newHeight = $("#"+id+"_CONTAINER").parent().height();
					newHeight -= $("#"+id+"_CTRL").outerHeight(true);	
					$("#"+id+"_CONTAINER").css("height",newHeight-8);
				};
				...
				this.dfPagerUI.pagerResize[id] = SearchContainerSize;
